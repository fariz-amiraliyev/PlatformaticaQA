...
public class GroupQQRTest extends BaseTest {
    ...
    @Test
    public void evgenRioFindRepo() throws InterruptedException {

        WebDriver browser = getDriver();
        browser.get("https://github.com/disel20/java");

        WebElement button = browser.findElement(By.xpath("//get-repo"));
        button.click();

        Thread.sleep(1000);

        WebElement name = browser.findElement(By.xpath("//input/a[contains(text(), 'Download ZIP')]"));
        Assert.assertEquals(name.getText(), "Download ZIP");
    }
}
